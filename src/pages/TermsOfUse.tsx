...
export default function TermsOfUse() {
        ...
        return (
            <div className="text-left w-full bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 px-6 py-8 rounded shadow max-w-5xl mx-auto text-sm">
                    ...
                    <p className="mb-1">{t("thirdparty.text2")}</p>
                    ...
                    <ul className="mb-2">
                            {(t("liability.items", { returnObjects: true }) as string[]).map((item, i) => (
                                <li className={listItem} key={i}>{item}</li>
                            ))}
                    </ul>

                    <h2 className={sectionTitle}>{t("user.title")}</h2>
                    <ul className="mb-2">
                            {(t("user.items", { returnObjects: true }) as string[]).map((item, i) => (
                                <li className={listItem} key={i}>{item}</li>
                            ))}
                    </ul>
