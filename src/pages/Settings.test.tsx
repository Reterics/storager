...
vi.mock('../components/elements/StyledInput', () => ({
    ...
    default: ({ label, name, value, onChange, type }: StyledInputArgs) => (
        <div>
            <label id={name + '_label'} htmlFor={name}>{label}</label>
            {type === 'checkbox' ? (
                <input id={name}
                       aria-labelledby={label ? name + '_label' : undefined}
                       type="checkbox" name={name} checked={!!value} onChange={onChange} />
            ) : type === 'textarea' ? (
                <textarea
                    aria-labelledby={label ? name + '_label' : undefined}
                    id={name}
                    name={name} value={value} onChange={onChange as unknown as ChangeEventHandler<HTMLTextAreaElement>} />
            ) : (
                <input
                    id={name}
                    aria-labelledby={label ? name + '_label' : undefined}
...
describe('Settings Component', () => {
    ...
    it('renders UnauthorizedComponent when user is not authorized', () => {
        ...
        expect(screen.getByText('401 Unauthorized - Your privileges has been revoked')).toBeInTheDocument();
    });
    ...
    it('renders form with initial settings when user is authorized', () => {
        ...
        expect(screen.getByLabelText('Bank Account Number')).toHaveValue('123456789');
        expect(screen.getByLabelText('Phone')).toHaveValue('555-1234');
        expect(screen.getByLabelText('Email')).toHaveValue('test@example.com');
        expect(screen.getByLabelText('Service Agreement')).toHaveValue('Test Agreement');
    ...
    it('handles textarea input correctly', () => {
        ...
        fireEvent.change(serviceAgreementTextarea, { target: { value: 'Updated Agreement' } });
