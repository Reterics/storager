...
export default function Logs() {
    ...
    const deleteLog = async (item: LogEntry) => {
        if (item.id && window.confirm(t('Are you sure you wish to delete this Log?'))) {
            const updatedItems = await dbContext?.removeData('logs', item.id) as LogEntry[];
...
`.trim();
            ...
            >{browserInfo?.name || log.user_agent?.substring(0,11)} {browserInfo?.isMobile ? '(Mobile)' : '(Web)'}</span>,
            log.changes ? formatChanges(log.changes as { [key: string]: { from: string; to: string } }) : 'No changes',
            assignedShops?.length ? assignedShops.map(a=>a.name).join(', ') : t('Minden bolt'),
            new Date(log.at || log.docUpdated!).toISOString().replace(/T/, " ").split('.')[0],
            TableViewActions({
