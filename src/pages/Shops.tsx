...
function Shops() {
  ...
  const deleteShop = async (shop: Shop) => {
    if (
      shop.id &&
      window.confirm(t('Are you sure you wish to delete this Shop?'))
    ) {
  ...
  const tableLines = shops.map((shop) => {
    ...
    if (
      dbContext?.data.currentUser?.role ===
      'admin' /*&& there is no data for this shop*/
    ) {
      actions.onPaste = () => {
        setImportShop(shop);
      };
    }
    return [shop.name || '', shop.address || '', TableViewActions(actions)];
  });
  ...
  return (
    <>
      <PageHead
        title={
          shopContext.shop
            ? shopContext.shop.name + t(' selected')
            : t('Select a Shop')
        }
        buttons={
          isAdmin
