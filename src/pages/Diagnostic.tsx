...
function sortInconsistencyResultsByLastOccurrenceDate(results: InconsistencyResult[]): InconsistencyResult[] {
...
function markInconsistentOccurrences(results: InconsistencyResult[]): InconsistencyResult[] {
...
function findClientNameInconsistencies(
    ...
    for (const form of completionForms) {
        if (form.service_id && form.client_name) {
            ...
            if (!occurrencesMap[form.service_id]) occurrencesMap[form.service_id] = [];
    ...
    const results: InconsistencyResult[] = Object.keys(occurrencesMap).map(service_id => {
...
function Diagnostic() {
    ...
    const [servicedItems] = useState<ServiceData[]>(dbContext?.data.services || []);
    const [completionForms] = useState<ServiceCompleteData[]>(dbContext?.data.completions || []);

    const [modalTemplate] = useState<ServiceData|null>(null);

    const inconsistencies = markInconsistentOccurrences(
        findClientNameInconsistencies(servicedItems, completionForms, dbContext?.data.archive || []));
    ...
    const sinceLastBuild = inconsistencies.reduce((total, inconsistency)=> {
        total+=inconsistency.occurrences.filter(o => new Date(o.date ?? 0).getTime() > buildTime).length ? 1 : 0;
        return total;
    }, 0);

    const tableLines = inconsistencies.reduce((tableLines, inconsistency) => {
        const serviceItem = servicedItems.find(service => service.id === inconsistency.service_id);
        const issueAfterRelease = inconsistency.occurrences.filter(o => new Date(o.date ?? 0).getTime() > buildTime).length

        inconsistency.occurrences.forEach((occurence, index) => {
            tableLines.push([
                !index ? inconsistency.service_id : '',
                occurence.inconsistentOccurrence ? <div className={issueAfterRelease ?
                    "w-full h-full bg-red-200 text-gray-800" : "w-full h-full bg-amber-200 text-gray-800"
                }>{occurence.client_name}</div> : occurence.client_name,
    ...
    return (
        <>
            {noModalActive && <PageHead title={t('Service Diagnostic')} error={sinceLastBuild + t(' inconsistencies since last build')}/>}
