...
import {defaultContextData, defaultItems, defaultParts} from "../../tests/mocks/shopData.ts";
...
describe('Items', () => {
    ...
    it('filters parts based on search input', async () => {
        const {container, getByPlaceholderText, unmount} = render(<TestingPageProvider><Items /></TestingPageProvider>);
        ...
        await waitFor(() =>
            expect(container.querySelectorAll('table > tbody > tr').length).toBeLessThan(defaultItems.length)
        )
    ...
    it('displays a low storage warning alert', () => {
        ...
        const {getByText, unmount} = render(<TestingPageProvider ctxDataOverride={ctxDataOverride}><Items /></TestingPageProvider>);
    ...
    it('deletes a part upon confirmation', async () => {
        const {container,unmount} = render(
            <TestingPageProvider removeData={
                vi.fn(async (): Promise<ContextDataCollectionType | null> => [defaultItems[0]])
            }
            >
                <Items />
            </TestingPageProvider>
