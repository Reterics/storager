...
export default class ImageDBModel extends DBModel {
    ...
    async loadPersisted() {
        ...
        this._imageStore = localImages.reduce((sum, image) => {
            if (sum['screenshots/' + image] && !sum['screenshots/' + image].url.startsWith('./uploads')) {
                sync = true;
            }
    ...
    decodeStorageImageURL(url: string) {
        try {
            return './uploads/' + decodeURIComponent(url).split('screenshots/')[1].split('?alt=media&token')[0];
        } catch (e) {
