...
import {getFirestore, collection, getDocs, getDoc, doc, setDoc, deleteDoc, addDoc,
    DocumentSnapshot, DocumentReference} from 'firebase/firestore';
...
describe('FirebaseDBModel', () => {
    ...
    describe('get', () => {
        it('should fetch and cache data if the document exists', async () => {
            const mockDocSnap = { exists: () => true, data: () => ({ id: '123', name: 'Test Item' }) };
            vi.mocked(getDoc).mockResolvedValue(mockDocSnap as unknown as DocumentSnapshot);
            ...
            expect(model.getCachedEntry('123', 'items')).toEqual({ id: '123', name: 'Test Item' });
        });
    ...
    describe('remove', () => {
        it('should mark the document as deleted and move it to the deleted cache', async () => {
            ...
            expect(model.getCachedEntry(id, 'deleted')).toEqual(expect.objectContaining({ deleted: true }));
    ...
    describe('restore', () => {
        it('should restore a deleted document to its original table', async () => {
            const id = '123';
            model.updateCache('deleted', [{ id, name: 'Deleted Item', docType: 'items', deleted: true }]);
            vi.mocked(setDoc).mockResolvedValue(undefined);

            const result = await model.restore(id);
            expect(doc).toHaveBeenCalledWith(model.getDB(), 'items', id);
            expect(setDoc).toHaveBeenCalledWith(expect.any(Object), expect.objectContaining({ deleted: false }), { merge: true });
            expect(result).toBe(true);
            expect(model.getCachedEntry(id, 'deleted')).toBeUndefined();
            expect(model.getCachedEntry(id, 'items')).toEqual(expect.objectContaining({ deleted: false }));
        });
    });
    ...
    describe('update', () => {
        it('should update a document in Firestore and cache', async () => {
            const item = { id: '123', name: 'Updated Item', docUpdated: 0, image: 'https://firebase' };
            const table = 'items';
            ...
            expect(setDoc).toHaveBeenCalledWith(expect.any(Object), expect.objectContaining({ name: 'Updated Item' }), { merge: true });
            expect(model.getCachedEntry('123', table)).toEqual(expect.objectContaining({ name: 'Updated Item' }));
        });
    });
    ...
    describe('add', () => {
        it('should add a new document to Firestore and cache', async () => {
            ...
            expect(model.getCachedEntry('new123', table)).toEqual(expect.objectContaining({ id: 'new123', name: 'New Item' }));
