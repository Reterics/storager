...
vi.mock('../elements/PageHead.tsx', () => ({
    PageHead: ({ buttons, onSearch }: {buttons: GeneralButtons[], onSearch?: (value: string) => void}) => (
        <div data-testid="PageHead">
            ...
            <input
                type="text"
                placeholder="Search"
                onChange={(e) => typeof onSearch === 'function' ? onSearch(e.target.value) : null}
            />
        </div>
    ),
}));
...
describe('ImportShopData Component', () => {
    ...
    const mockDBContext = {
        data: {
            items: [
                { sku: 'sku1', name: 'Item 1', storage_limit: [10], price: 100, shop_id: [] },
                { sku: 'sku2', name: 'Item 2', storage_limit: [5], price: 200, shop_id: [] },
            ],
            parts: [
    ...
    it('filters data based on search input', async () => {
        ...
        fireEvent.change(screen.getByPlaceholderText('Search'), { target: { value: 'Item 1' } });
