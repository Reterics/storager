...
describe('TypeModal', () => {
    ...
    it('renders without crashing when type is null', () => {
        const { container } = render(<TypeModal
            type={null}
            onClose={onClose}
            onSave={onSave}
            setType={setType}
    ...
    it('updates type name on input change', () => {
        setType.mockReset();
        const untranslatedMock = {...mockType, translations: undefined, category: undefined};
        render(<TypeModal
        ...
        expect(setType).toHaveBeenCalledWith({ ...untranslatedMock, name: 'New Type Name', translations: { en: 'New Type Name' } });

        const categoryInput = screen.getByLabelText('Category');
        fireEvent.change(categoryInput, { target: { value: 'item' } });
        expect(setType).toHaveBeenCalledWith({ ...untranslatedMock, category: 'item' });

        const enInput = screen.getByLabelText('EN');
        fireEvent.change(enInput, { target: { value: 'item' } });
        expect(setType).toHaveBeenCalledWith({ ...untranslatedMock, translations: { en: 'item' } });

        const huInput = screen.getByLabelText('HU');
        fireEvent.change(huInput, { target: { value: 'item' } });
        expect(setType).toHaveBeenCalledWith({ ...untranslatedMock, translations: { hu: 'item' } });
    });
});
