...
export default function TableSelectComponent<T>(
    ...
    return (
        <table
            className="w-full table-auto text-sm text-left text-gray-700 dark:text-gray-200 border rounded shadow-md">
                ...
                return (
                    ...
                    >
                        {itemRenderer(item).map((col, idx) => (
                            <td key={idx} className="px-3 py-2 whitespace-nowrap">{col}</td>
                        ))}
            ...
            {!items.length && (
                <tr>
                    <td colSpan={headers.length + 2} className="px-4 py-6 text-center text-gray-400 dark:text-gray-500">
                        No data available.
                    </td>
                </tr>
            )}
